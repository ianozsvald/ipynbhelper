"""Hackish script to convert a notebook into asciidoc
...
To specify a directory (default is '.') use:

  $ python ipynbhelper.py --render --convert --output_directory output notebooks/chapter-1-introduction.ipynb
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Convert IPython Notebook to asciidoc')
    parser.add_argument('--convert', '-c', action="store_true", help='Convert to asciidoc', default=False)
    parser.add_argument('--render', '-r', action="store_true", help='Re-render the Notebook before conversion', default=False)
    parser.add_argument('--output-directory', '-d', type=str, help='Directory to store the asciidoc form', default=".")
    parser.add_argument('targets', nargs="*", help="One or more target files to convert to asciidoc")
    ...
    if not targets:
        targets = [os.path.join(os.path.dirname(__file__), DEFAULT_TARGET)]
        print("Defaulting to use {}".format(targets))
    ...
    for target in targets:
        ...
        for fname in fnames:
            if 'merge' in actions or 'convert' in actions:
                if fname.endswith(NOTEBOOK_EXTENSION):
                    asciidoc_fname = os.path.basename(fname)[:-len(NOTEBOOK_EXTENSION)]
                else:
                    asciidoc_fname = os.path.basename(fname)
                ...
                asciidoc_fname = None
            print("Working on {}, writing to {}".format(fname, asciidoc_fname))
